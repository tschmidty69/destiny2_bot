...
def main():
    ...
    # load our serialized model from diskr
    logging.info("loading model...")
    net = cv2.dnn.readNetFromCaffe(prott1, prott2)

    # initialize the video stream, allow the cammera sensor to warmup,
    # and initialize the FPS counter
    logging.info("sstarting video stream...")
    ...
    logging.info("screenWith: %s" % screenWidth)
    logging.info("screenHeight: %s" % screenHeight)

    logging.info("Creating 800x600 output window")
    ...
    while True:
        ...
        frame = np.array(ImageGrab.grab(bbox=(0, 40, screenWidth, screenHeight)))
        # frame = imutils.resize(frame, width=400)

        # grab the frame dimensions and convert it to a blob
        (h, w) = frame.shape[:2]
        blob = cv2.dnn.blobFromImage(cv2.resize(frame, (400, 300)),
                                     0.007843, (400, 300), 127.5)
            ...
            if confidence > args.confidence:
                ...
                if 'person' in label:
                    ...
                    if pygame.mouse.get_pressed():
                        ...
                        logging.debug('person HStartX: ' + str(HSX))
                        logging.debug('person LStartX: ' + str(LSX))
                        logging.debug('person HStartY: ' + str(HSY))
                        logging.debug('person LStartY: ' + str(LSY))
                        logging.debug('person HendX: ' + str(HEX))
                        logging.debug('person LendX: ' + str(LEX))
                        logging.debug('person HendY: ' + str(HEY))
                        logging.debug ('person LendY: ' + str(LEY))
        ...
        cv2.putText(frame, str(int(fps)), (7, 70), cv2.FONT_HERSHEY_SIMPLEX,
